...
marker_styles = ["o", "+", "*", ".","x","square","|", "square", "diamond", "^"];
line_styles = ["-", "--", ":", "-.","-", "--", ":", "-.","-", "--" ];
...
plot(XVec, YVec, 'LineWidth', 2, 'LineStyle', line_styles(node), 'color', string(colours(node)))
hold on
if node == 6
plot(XVec(1:10:end),YVec(1:10:end),marker_styles(node), 'color', string(colours(node)), 'MarkerSize',10, 'MarkerFaceColor', string(colours(node)),'linestyle', 'none')
else
    if node == 4
        
    plot(XVec(1:10:end),YVec(1:10:end), marker_styles(node), 'color', string(colours(node)), 'MarkerSize',24,'linestyle', 'none')
    else
        plot(XVec(1:10:end),YVec(1:10:end), marker_styles(node), 'color', string(colours(node)), 'MarkerSize',10,'linestyle', 'none')
    end
end
